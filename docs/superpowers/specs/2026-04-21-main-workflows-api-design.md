...
| Update / delete | `PUT` `DELETE` | `/api/check-ins/{id}` | **OK** | [`2026-04-21-check-in-update-delete-author-only.md` (done)](../../tickets/done/2026-04-21-check-in-update-delete-author-only.md) | **Bearer JWT** (**401** without). Same **read** gate as list/get check-ins (**404** if viewer is not owner/participant with **`findByIdForViewer`** semantics). Only **check-in author** may mutate (**403** for others). Missing id → **404**. |
