...
- **`POST /api/invites`:** **Bearer JWT** required (**401** without). Body **`InviteCreateRequest`**: **`inviteeEmail`** (must match a registered user, case-insensitive), **`challengeId`**, optional **`subTaskId`**, **`status`**, **`expiresAt`**. **Inviter** is always the authenticated user. Only the **challenge owner** may create invites (**403** for others). Unknown **`inviteeEmail`** → **404**; inviting yourself → **403**.
...
- **`GET /api/challenges/{id}/check-ins`**, **`GET /api/check-ins/{id}`:** **Bearer JWT** required — anonymous callers get **401** from **`SecurityConfig`**; with a token, viewer must be **owner** or any **participant** of the challenge, and the challenge must pass **`findByIdForViewer`** (a **usable `PENDING` invite** alone does **not** grant check-in read). Otherwise **404**. Rolled-up **`check_in_summaries`** remain internal to **`CheckInRollupService`**; there is **no** HTTP list for them.
...
- **`PUT /api/check-ins/{id}`**, **`DELETE /api/check-ins/{id}`:** **Bearer JWT** required (**401** without). Viewer must pass the same check-in **read** gate as **`GET /api/check-ins/{id}`** (**404** if not owner or participant with **`findByIdForViewer`** semantics). Only the **check-in author** (**`CheckIn.user`**) may update or delete (**403** for others, including challenge **owner** on another user’s row). Missing check-in → **404**.
- **Authentication:** **JWT** — default is **`/api/**` authenticated**; **`SecurityConfig`** allowlists **`POST /api/login`**, **`POST /api/register`**, **`POST /api/users`**, forgot/reset password, **`GET /api/email/verify/**`**, **public discovery** **`GET`**s (`/api/categories`, **`GET /api/challenges`** list, **`GET /api/challenges/{numericId}`**, **`GET /api/challenges/{id}/subtasks`**, **`GET /api/challenges/{id}/participants`**, **`GET /api/subtasks/{id}`**), plus **`/actuator/**`** and **springdoc** (`/swagger-ui.html`, `/v3/api-docs`, …). See **`SecurityConfig`** for the exact ordered matchers.
