...
- **`PUT /api/check-ins/{id}`**, **`DELETE /api/check-ins/{id}`:** **Product rule** — only the **check-in author** (the user on the check-in row) may update or delete; not implemented yet — see [`docs/tickets/2026-04-21-check-in-update-delete-author-only.md`](docs/tickets/2026-04-21-check-in-update-delete-author-only.md).
