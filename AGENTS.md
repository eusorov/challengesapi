...
- **`GET /api/challenges/{id}/check-ins`**, **`GET /api/check-ins/{id}`:** **Bearer JWT** required (**404** if missing). Viewer must be **owner** or any **participant** of the challenge, and the challenge must pass **`findByIdForViewer`** for the challenge context (a **usable `PENDING` invite** alone does **not** grant check-in read). Otherwise **404**. Rolled-up **`check_in_summaries`** remain internal to **`CheckInRollupService`**; there is **no** HTTP list for them.
